...
class BlockingCommitThenAbortCloseStream(httpx.AsyncByteStream):
    def __init__(self, error: BaseException) -> None:
        self.error = error
        self.started = anyio.Event()
        self.release = anyio.Event()
...
@pytest.mark.anyio
async def test_waiter_retries_after_cleanup_commits_then_control_flow_aborts() -> None:
    error = ControlFlowAbort("cleanup committed before caller abort")
    stream = BlockingCommitThenAbortCloseStream(error)
    response = httpx.Response(200, stream=stream)
    owner_errors: list[BaseException] = []
    waiter_completed = anyio.Event()
    ...
    assert response.is_closed is True
    assert stream.close_calls == 2
    assert stream.cleanup_commits == 2
